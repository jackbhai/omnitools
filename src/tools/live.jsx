      ...
      <div className="list" style={{ marginTop: 10 }}>
        {g.data.slice(0, 5).map((r, i) => (
          <button key={i} className="col" style={{ background: 'none', border: 0, textAlign: 'left', width: '100%', cursor: 'pointer' }}
            onClick={() => { setLoc({ lat: r.lat, lon: r.lon, name: r.name }); setQ(''); }}>
            <b>{r.name}</b><span className="dim sm">{[r.admin, r.country].filter(Boolean).join(', ')}</span>
          </button>))}
      </div>)}
    ...
    {w.data && (<>
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ fontSize: 54, lineHeight: 1 }}>{wmo(w.data.code)[1]}</div>
          <div>
            <div className="big">{fmt(w.data.temp, 1)}<span style={{ fontSize: 20 }}>°C</span></div>
        ...
        <div className="g3" style={{ marginTop: 14 }}>
          ...
          <Stat l="Wind" v={fmt(w.data.wind, 1)} s="km/h" />
          <Stat l="Rain" v={fmt(w.data.precip, 1)} s="mm" />
        </div>
        {w.data.sunrise && (
          <div className="g2" style={{ marginTop: 8 }}>
            <Stat l="Sunrise" v={new Date(w.data.sunrise).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})} />
            <Stat l="Sunset" v={new Date(w.data.sunset).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})} />
          </div>)}
        <Src meta={w.meta} />
      </Card>
      ...
      {a.data && (
        <Card>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
            <div className="big" style={{ color: tone(a.data.aqi) }}>{fmt(a.data.aqi)}</div>
            <div className="dim sm">AQI{a.data.station ? ` · ${a.data.station}` : ''}</div>
          </div>
          <div className="g3" style={{ marginTop: 12 }}>
            <Stat l="PM2.5" v={fmt(a.data.pm25, 1)} s="µg/m³" />
            <Stat l="PM10" v={fmt(a.data.pm10, 1)} s="µg/m³" />
            <Stat l="Ozone" v={fmt(a.data.o3, 1)} s="µg/m³" />
          </div>
          <Src meta={a.meta} />
        </Card>)}

      <div className="chead" style={{ marginTop: 16 }}>7-day forecast</div>
      <div className="list">
        {w.data.daily.map((d, i) => (
          <div className="row" key={i}>
            ...
            <span style={{ fontSize: 19 }}>{wmo(d.code)[1]}</span>
            {d.pop != null && <span style={{ color: 'var(--cyan)', fontSize: 11, width: 34, textAlign: 'right' }}>{d.pop}%</span>}
            <span style={{ fontSize: 13.5, width: 74, textAlign: 'right' }}>
              <b>{fmt(d.max)}°</b> <span className="dim">{fmt(d.min)}°</span></span>
          </div>))}
      </div>
    </>)}
  </>);
...
export function WorldClock() {
  ...
  return (<>
    <Card>
      <div className="chead">🇮🇳 India Standard Time</div>
      <div className="big gradtext mono">
      ...
      <div className="src"><span className="dot" /><span>Live · runs offline (Intl API)</span></div>
    </Card>
    <div className="list" style={{ marginTop: 12 }}>
      {ZONES.map(([tz, label]) => {
        ...
        return (
          <div className="row" key={tz}>
            <div className="main"><b>{label}</b><span className="dim sm">{d} · {off}</span></div>
            <div className="end mono" style={{ fontSize: 16, fontWeight: 700 }}>{t}</div>
          </div>);
      })}
    </div>
  </>);
...
export function Festivals() {
  ...
  return (<>
    <div className="cats">{CCS.map(([c, l]) =>
      <button key={c} className={`cat ${cc===c?'on':''}`} onClick={() => setCc(c)}>{l}</button>)}</div>
    <div className="btnrow">{[year-1, year, year+1].map((y) =>
      <button key={y} className={`cat ${year===y?'on':''}`} onClick={() => setYear(y)}>{y}</button>)}</div>
    {h.loading && <Spin t="Loading festivals" />}
    {h.error && <Err error={h.error} retry={() => h.run()} />}
    {h.data && (<>
      {next && (
        <Card style={{ marginTop: 12 }}>
          <div className="chead">Next festival</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 30, letterSpacing: 1 }} className="gradtext">{next.name}</div>
      ...
      <div className="dim sm" style={{ margin: '12px 0 8px' }}>{h.data.length} holidays in {year}</div>
      <div className="list">
        {h.data.map((x, i) => {
          const past = x.date < today;
          const d = new Date(x.date);
          return (
            <div className="row" key={i} style={{ opacity: past ? .38 : 1 }}>
              <div style={{ width: 46, textAlign: 'center', background: 'var(--s2)', borderRadius: 10, padding: '6px 0', flex: '0 0 auto' }}>
                <b style={{ display: 'block', fontSize: 16, lineHeight: 1 }}>{d.getDate()}</b>
              ...
              <div className="main"><b>{x.name}</b>
                <span className="dim sm">{d.toLocaleDateString('en', { weekday: 'long' })}</span></div>
            </div>);
        })}
      </div>
      <Src meta={h.meta} />
...
export function OnThisDay() {
  ...
  return (<>
    ...
    <Chips items={['Event','Born','Died','Holiday']} value={kind} onPick={setKind} />
    {o.loading && <Spin />}
...
export function Currency() {
  ...
  return (<>
    <div className="g2">
      ...
      <Field label="From" as="select" value={base} onChange={(e)=>setBase(e.target.value)}>
    ...
    {c.data && (
      <Card>
        <div className="chead">{fmt(n,2)} {c.data.base} equals</div>
        {FAV.filter((x)=>x!==base && c.data.rates[x]).map((x)=>(
          <div className="kv" key={x}>
            <span style={{fontWeight:700,color:'var(--fg)'}}>{x}</span>
            <b style={{fontSize:16}}>{fmt(n*c.data.rates[x],2)}
              <span className="dim sm" style={{fontWeight:400,marginLeft:8}}>@{fmt(c.data.rates[x],4)}</span></b>
          </div>))}
        <div className="dim sm" style={{marginTop:8}}>Rate date: {c.data.date}</div>
        <Src meta={c.meta} />
      </Card>)}
  </>);
}
...
export function Crypto() {
  ...
  return (<>
    <div className="btnrow"><button className="btn ghost sm" onClick={()=>c.run()}>↻ Refresh</button></div>
    ...
    {c.data && (<>
      <div className="list">
        {c.data.map((x,i)=>(
          <div className="row" key={i}>
            <span className="dim mono" style={{width:22,fontSize:11}}>{x.rank}</span>
            <div className="main"><b>{x.sym}</b><span className="dim sm">{x.name}</span></div>
            <div className="end">
              <b>${fmt(x.price, x.price<1?6:2)}</b><br/>
              <span style={{color:x.change>=0?'var(--green)':'var(--bad)',fontSize:11.5}}>
                {x.change>=0?'▲':'▼'} {fmt(Math.abs(x.change),2)}%</span>
            </div>
          </div>))}
      </div>
      <Src meta={c.meta} />
...
export function Pincode() {
  ...
  return (<>
    <Search value={q} onChange={setQ} onSubmit={go} ph="6-digit PIN code" />
    <div className="btnrow">{['110001','143001','400001','560001','700001'].map((x)=>
      <button key={x} className="cat" onClick={()=>{setQ(x);p.run({q:x});}}>{x}</button>)}</div>
    {p.loading && <Spin />}
    {p.error && <Err error={p.error} retry={go} />}
    {p.data && (<>
      <div className="dim sm" style={{margin:'10px 0'}}>{p.data.length} post office(s)</div>
      <div className="list">{p.data.map((x,i)=>(
        <div className="col" key={i}><b>{x.name}</b>
          <span className="dim sm">{x.branch} · {x.district}, {x.state}</span>
...
export function Ifsc() {
  ...
  return (<>
    <Search value={q} onChange={(v)=>setQ(v.toUpperCase())} onSubmit={go} ph="IFSC e.g. HDFC0000001" />
    <div className="btnrow">{['HDFC0000001','SBIN0000001','ICIC0000001'].map((x)=>
      <button key={x} className="cat" onClick={()=>{setQ(x);f.run({q:x});}}>{x}</button>)}</div>
    {f.loading && <Spin />}
    {f.error && <Err error={f.error} retry={go} />}
    {f.data && (
      <Card>
        <div className="chead">{f.data.bank}</div>
        <div className="kv"><span>Branch</span><b>{f.data.branch}</b></div>
        <div className="kv"><span>IFSC</span><b className="mono">{f.data.ifsc}</b></div>
        {f.data.micr && <div className="kv"><span>MICR</span><b className="mono">{f.data.micr}</b></div>}
        <div className="kv"><span>City</span><b>{f.data.city}</b></div>
        <div className="kv"><span>State</span><b>{f.data.state}</b></div>
        <div className="kv"><span>Address</span><b style={{fontWeight:400,fontSize:12.5}}>{f.data.address}</b></div>
        <div className="btnrow">{[['UPI',f.data.upi],['NEFT',f.data.neft],['IMPS',f.data.imps],['RTGS',f.data.rtgs]]
          .map(([k,v])=><span key={k} className={`tag ${v?'g':''}`}>{k} {v?'✓':'✕'}</span>)}</div>
        <Src meta={f.meta} />
      </Card>)}
  </>);
}
...
function SearchTool({ cap, pool, ph, presets = [], render, initial = '' }) {
  ...
  return (<>
    <Search value={q} onChange={setQ} onSubmit={go} ph={ph} />
    {presets.length > 0 && <div className="btnrow">{presets.map((x)=>
...
export const Wikipedia = () => <SearchTool cap="wiki" pool={P.wiki} ph="Search Wikipedia…"
  ...
  <div className="list">{d.map((p,i)=>(
    <a className="col" key={i} href={p.url} target="_blank" rel="noreferrer">
      <b>{p.title}</b>{p.desc && <span className="dim sm">{p.desc}</span>}
...
export const Dictionary = () => <SearchTool cap="dict" pool={P.dictionary} ph="Look up a word…"
  ...
  <Card>
    <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',marginBottom:12}}>
      <b style={{fontSize:24,fontFamily:'var(--font-display)',letterSpacing:1}}>{d.word}</b>
    ...
    {d.meanings.map((m,i)=>(
      <div key={i} style={{marginBottom:14}}>
        <div style={{fontStyle:'italic',color:'var(--cyan)',fontSize:12.5,marginBottom:6}}>{m.pos}</div>
...
export const News = () => <SearchTool cap="news" pool={P.news} ph="Search tech news…" initial=" "
  render={(d)=>(<div className="list">{d.map((h,i)=>(
    <a className="col" key={i} href={h.url} target="_blank" rel="noreferrer">
      <b>{h.title}</b>
      <span className="dim sm">▲ {h.points} · 💬 {h.comments} · {h.author} · {h.date}</span></a>))}</div>)} />;
    ...
    {d.map((m,i)=>(
      <div className="tile" key={i} style={{padding:0,overflow:'hidden',minHeight:0,display:'block'}}>
        {m.img ? <img src={m.img} alt="" loading="lazy" style={{width:'100%',aspectRatio:'2/3',objectFit:'cover'}} />
          : <div style={{aspectRatio:'2/3',display:'grid',placeItems:'center',fontSize:30,background:'var(--s2)'}}>🎬</div>}
        <div style={{padding:9,textAlign:'left'}}>
          <b style={{fontSize:12,display:'block'}}>{m.title}</b>
          <span className="dim" style={{fontSize:10.5}}>{String(m.year).slice(0,4)}{m.rating?` · ⭐ ${m.rating}`:''}</span>
        </div>
      </div>))}
  </div>)} />;
    ...
    {d.map((b,i)=>(
      <a className="tile" key={i} href={b.url} target="_blank" rel="noreferrer"
        ...
        {b.cover ? <img src={b.cover} alt="" loading="lazy" style={{width:'100%',aspectRatio:'2/3',objectFit:'cover'}} />
          : <div style={{aspectRatio:'2/3',display:'grid',placeItems:'center',fontSize:30,background:'var(--s2)'}}>📖</div>}
        <div style={{padding:9,textAlign:'left'}}>
          <b style={{fontSize:11.5,display:'block'}}>{b.title.slice(0,44)}</b>
          <span className="dim" style={{fontSize:10}}>{b.author}</span>
        </div>
      </a>))}
  </div>)} />;
...
export const Repos = () => <SearchTool cap="gh" pool={P.github} ph="Search GitHub…"
  ...
  <div className="list">{d.map((r,i)=>(
    <a className="col" key={i} href={r.url} target="_blank" rel="noreferrer">
      <b>{r.name}</b>{r.desc && <span className="dim sm">{r.desc}</span>}
...
export const Countries = () => <SearchTool cap="country" pool={P.country} ph="Search a country…"
  ...
  <>{d.map((c,i)=>(
    <Card key={i}>
      <div className="chead">{c.flag && <img src={c.flag} alt="" style={{width:26,height:18,borderRadius:3,objectFit:'cover'}} />} {c.name}</div>
...
export function Space() {
  ...
  return (<>
    <Card>
      <div className="chead">🛰️ ISS live position</div>
      {i.loading && !i.data && <Spin t="Tracking" />}
      {i.error && <Err error={i.error} retry={()=>i.run()} />}
      {i.data && (<>
        <div className="g2">
          <Stat l="Latitude" v={fmt(i.data.lat,3)} /><Stat l="Longitude" v={fmt(i.data.lon,3)} />
        </div>
        {i.data.alt != null && <div className="g2" style={{marginTop:8}}>
          <Stat l="Altitude" v={fmt(i.data.alt,1)} s="km" /><Stat l="Speed" v={fmt(i.data.vel)} s="km/h" /></div>}
...
export function Quakes() {
  ...
  return (<>
    ...
    {q.data && (<>
      <div className="list">{q.data.map((x,i)=>(
        <a className="row" key={i} href={x.url} target="_blank" rel="noreferrer">
          <div style={{width:44,height:44,borderRadius:11,display:'grid',placeItems:'center',flex:'0 0 auto',
            ...
            {x.mag?.toFixed(1)}</div>
          <div className="main"><b style={{fontSize:13}}>{x.place}</b>
            <span className="dim sm">{new Date(x.time).toLocaleString('en-IN')} · {fmt(x.depth,1)} km deep</span></div>
        </a>))}</div>
      <Src meta={q.meta} />
    </>)}
  </>);
...
export function MyIp() {
  ...
  return (<>
    ...
    {i.error && <Err error={i.error} retry={()=>i.run()} />}
    {i.data && (<Card>
      <div className="chead">Your connection</div>
      <div className="big mono gradtext" style={{fontSize:26}}>{i.data.ip}</div>
      {i.data.city && <>
        <div className="kv" style={{marginTop:12}}><span>Location</span><b>{[i.data.city,i.data.region,i.data.country].filter(Boolean).join(', ')}</b></div>
        <div className="kv"><span>ISP</span><b style={{fontSize:12.5}}>{i.data.org}</b></div>
        <div className="kv"><span>Timezone</span><b>{i.data.tz}</b></div>
      </>}
      <div className="btnrow"><Copy text={i.data.ip} label="Copy IP" /></div>
      <Src meta={i.meta} />
    </Card>)}
  </>);
}
...
export function Jokes() {
  ...
  return (<Card>
    ...
    {j.data && (<>
      <div style={{fontSize:16,lineHeight:1.5}}>{j.data.setup}</div>
      <div className="gradtext" style={{fontSize:18,fontWeight:700,marginTop:12}}>{j.data.punch}</div>
      <button className="btn" style={{marginTop:14,width:'100%'}} onClick={()=>j.run()}>😄 Another</button>
      <Src meta={j.meta} />
    </>)}
  </Card>);
}
...
export function Quotes() {
  ...
  return (<Card>
    ...
    {q.error && <Err error={q.error} retry={()=>q.run()} />}
    {q.data && (<>
      <div style={{fontSize:17,lineHeight:1.55,fontStyle:'italic'}}>"{q.data.text}"</div>
      <div className="dim" style={{marginTop:10}}>— {q.data.author}</div>
      <div className="btnrow"><button className="btn sm" onClick={()=>q.run()}>↻ New</button>
        <Copy text={`"${q.data.text}" — ${q.data.author}`} /></div>
      <Src meta={q.meta} />
    </>)}
  </Card>);
}
...
export function NameGuess() {
  ...
  return (<>
    <Search value={q} onChange={setQ} onSubmit={()=>q.trim()&&n.run({q:q.trim()})} ph="Enter a first name…" />
    <div className="btnrow">{['Arjun','Priya','Ahmed','Simran'].map((x)=>
      <button key={x} className="cat" onClick={()=>{setQ(x);n.run({q:x});}}>{x}</button>)}</div>
    {n.loading && <Spin />}
    {n.error && <Err error={n.error} />}
    {n.data && (<Card>
      <div className="chead">Prediction for "{n.data.name}"</div>
      <div className="g2">
        <Stat l="Likely age" v={n.data.age ?? '—'} />
        <Stat l="Gender" v={n.data.gender ?? '—'} s={n.data.prob?`${Math.round(n.data.prob*100)}% sure`:''} />
      </div>
      <div className="dim sm" style={{marginTop:10}}>Based on {fmt(n.data.count)} records</div>
      <Src meta={n.meta} />
    </Card>)}
  </>);
}
