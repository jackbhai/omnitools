...
export function Manga() {
  ...
  return (<>
    <Search value={q} onChange={setQ} onSubmit={() => m.run({ q: q.trim() })} ph="Search manga… (blank = latest)" />
    <div className="btnrow">{['naruto','one piece','solo leveling','jujutsu'].map((x) =>
      <button key={x} className="cat" onClick={() => { setQ(x); m.run({ q: x }); }}>{x}</button>)}</div>
    {m.loading && <Spin t="Loading manga" />}
    {m.error && <Err error={m.error} retry={() => m.run({ q })} />}
    {m.data && (<>
      <div className="dim sm" style={{ margin: '10px 0 8px' }}>{m.data.length} titles</div>
        ...
        {m.data.map((x, i) => (
          <div className="tile" key={i} style={{ padding: 0, minHeight: 0, display: 'block', overflow: 'hidden' }}>
            {x.cover ? <img src={x.cover} alt="" loading="lazy"
              ...
              : <div style={{ aspectRatio: '2/3', display: 'grid', placeItems: 'center', fontSize: 28, background: 'var(--s2)' }}>📖</div>}
...
export function Novels() {
  ...
  return (<>
    <Search value={q} onChange={setQ} onSubmit={() => n.run({ q: q.trim() })} ph="Search novels…" />
    ...
    {n.data && (<>
      <div className="dim sm" style={{ margin: '10px 0 8px' }}>{n.data.length} novels</div>
      <div className="list">
        {n.data.map((x, i) => (
          <div className="row" key={i}>
            ...
            <div className="main"><b style={{ fontSize: 13 }}>{x.title}</b>
              ...
              {x.desc && <span className="dim sm">{x.desc}</span>}</div>
          </div>))}
      </div>
      <Src meta={n.meta} />
    </>)}
...
export function Medicine() {
  ...
  return (<>
    <Search value={q} onChange={setQ} onSubmit={() => q.trim() && m.run({ q: q.trim() })}
      ph="Medicine name, e.g. paracetamol" />
    <div className="btnrow">{['paracetamol','azithromycin','crocin','dolo 650'].map((x) =>
      <button key={x} className="cat" onClick={() => { setQ(x); m.run({ q: x }); }}>{x}</button>)}</div>
    {m.loading && <Spin t="Searching medicine database" />}
    {m.error && <Err error={m.error} retry={() => m.run({ q })} />}
    {m.data && (<>
      <div className="dim sm" style={{ margin: '10px 0 8px' }}>{m.data.length} results</div>
      {m.data.slice(0, 20).map((x, i) => (
        <Card key={i}>
          <div className="chead">{x.name || x.medicine || x.title || 'Medicine'}</div>
          {Object.entries(x).filter(([k, v]) =>
            v && typeof v !== 'object' && !['name','medicine','title'].includes(k)).slice(0, 8).map(([k, v]) => (
            <div className="kv" key={k}>
              <span>{k.replace(/_/g, ' ')}</span>
              <b style={{ fontSize: 12.5, fontWeight: 500, textAlign: 'right' }}>{String(v).slice(0, 120)}</b>
            </div>))}
        </Card>))}
      <div className="src"><span className="dot warn" />
        <span>Reference only — always consult a doctor or pharmacist.</span></div>
      <Src meta={m.meta} />
    </>)}
  </>);
}
...
export function Courses() {
  ...
  return (<>
    ...
    {c.data && (<>
      <Search value={q} onChange={setQ} ph={`Filter ${c.data.length} courses…`} />
      <div className="dim sm" style={{ marginBottom: 8 }}>{list.length} courses</div>
      <div className="list">
        {list.slice(0, 100).map((x, i) => (
          <a className="col" key={i} href={x.url || x.link} target="_blank" rel="noreferrer">
            <b style={{ fontSize: 13 }}>{x.name || x.title}</b>
...
export function Telenor() {
  ...
  return (<>
    ...
    {t.data && (<>
      <Card>
        <div className="chead">{t.data.title || 'My Telenor Quiz'}</div>
        <div className="dim sm">{t.data.date}</div>
      </Card>
      {t.data.questions.map((q, i) => (
        <Card key={i}>
          <b style={{ fontSize: 13.5 }}>Q{i + 1}. {q.question || q.q}</b>
          <div style={{ marginTop: 8, padding: 10, borderRadius: 10,
            background: 'rgba(0,255,156,.08)', border: '1px solid rgba(0,255,156,.25)' }}>
            <span className="dim sm">Answer</span>
            <b style={{ display: 'block', color: 'var(--green)', fontSize: 14 }}>{q.answer || q.a}</b>
          </div>
        </Card>))}
      <Src meta={t.meta} />
    </>)}
...
export function TempMail() {
  ...
  return (<>
    {!box && (
      <Card>
        <div className="chead">📨 Disposable email</div>
        <p className="dim sm">Generate a throwaway address to receive verification mails
          without giving away your real inbox.</p>
        <button className="btn" style={{ width: '100%', marginTop: 12 }} disabled={busy} onClick={create}>
          {busy ? 'Creating…' : '✉️ Create address'}</button>
      </Card>)}
    {err && <div className="err"><p>{err}</p></div>}
    {box && (<>
      <Card>
        <div className="chead">Your temporary address</div>
        <div className="out" style={{ fontSize: 14 }}>{box.email}</div>
        <div className="btnrow">
          <Copy text={box.email} label="Copy address" />
          <button className="btn ghost sm" disabled={busy} onClick={refresh}>
            {busy ? '…' : '↻ Check inbox'}</button>
          <button className="btn ghost sm" onClick={create}>New</button>
        </div>
      </Card>
      <div className="dim sm" style={{ margin: '10px 0 6px' }}>{inbox.length} message(s)</div>
      {inbox.length === 0
        ...
        : <div className="list">
            {inbox.map((m, i) => (
              <div className="col" key={i}>
                ...
                <span className="dim sm">{m.from || m.sender}</span>
                <span className="dim sm">{(m.intro || m.body || '').slice(0, 140)}</span>
              </div>))}
          </div>}
    </>)}
  </>);
...
export function WikiPdf() {
  ...
  return (<>
    <div className="fld"><label>Wikipedia article</label>
      <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="e.g. Babar Azam" /></div>
    <div className="btnrow">{['Delhi','Punjab','Cricket','ISRO'].map((x) =>
      <button key={x} className="cat" onClick={() => setQ(x)}>{x}</button>)}</div>
    <a className="btn" style={{ display: 'block', textAlign: 'center', marginTop: 12, textDecoration: 'none' }}
      href={`${B}wikipdf?query=${encodeURIComponent(q)}`} target="_blank" rel="noreferrer">
      📄 Generate &amp; open PDF</a>
    <div className="src"><span className="dot" /><span>via AHM7 Wikster — opens a real PDF</span></div>
  </>);
}
...
export function Handwriting() {
  ...
  return (<>
    <div className="fld"><label>Text to convert</label>
      <textarea value={text} onChange={(e) => setText(e.target.value)} /></div>
    <button className="btn" style={{ width: '100%' }} disabled={!text.trim()}
      onClick={() => setUrl(`${B}hand?text=${encodeURIComponent(text)}&_=${Date.now()}`)}>
      ✍️ Convert to handwriting</button>
    {url && (
      <Card style={{ marginTop: 12 }}>
        <img src={url} alt="handwriting" style={{ width: '100%', borderRadius: 10, background: '#fff' }} />
        <a className="btn sm" style={{ marginTop: 10, display: 'inline-block', textDecoration: 'none' }}
          href={url} download="handwriting.png" target="_blank" rel="noreferrer">⬇ Download PNG</a>
      </Card>)}
  </>);
}
...
export function WebSnap() {
  ...
  return (<>
    <div className="fld"><label>Website URL</label>
      <input value={u} onChange={(e) => setU(e.target.value)} placeholder="https://…" /></div>
    <button className="btn" style={{ width: '100%' }} disabled={!u.trim()}
      onClick={() => setShot(`${B}websnap?action=screenshot&url=${encodeURIComponent(u)}&_=${Date.now()}`)}>
      📸 Capture screenshot</button>
    {shot && (
      <Card style={{ marginTop: 12 }}>
        <img src={shot} alt="screenshot" style={{ width: '100%', borderRadius: 10 }} />
        <a className="btn sm" style={{ marginTop: 10, display: 'inline-block', textDecoration: 'none' }}
          href={shot} download="screenshot.jpg" target="_blank" rel="noreferrer">⬇ Download</a>
      </Card>)}
...
export function Certificates() {
  ...
  return (<>
    ...
    {c.error && <Err error={c.error} retry={() => c.run()} />}
    {c.data && (<>
      <div className="dim sm" style={{ marginBottom: 8 }}>{c.data.length} certificate templates</div>
      <div className="list">
        {c.data.map((t, i) => (
          <div className="row" key={i}>
            <span style={{ fontSize: 20 }}>🏆</span>
            <div className="main"><b>{t.name || t.id || `Template ${i + 1}`}</b>
              {t.description && <span className="dim sm">{t.description}</span>}</div>
          </div>))}
      </div>
      <div className="src"><span className="dot warn" />
        <span>Generation needs a POST request — browse templates here, generate via the AHM7 site.</span></div>
      <Src meta={c.meta} />
    </>)}
  </>);
}
