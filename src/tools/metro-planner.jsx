          ...
          {hits.map((h) => {
            ...
            return (
              <button key={h} className="col" style={{ background: 'none', border: 0,
                ...
                <span className="dim sm">{st?.l.join(' · ')}</span>
              </button>);
          })}
        </div>)}
    </div>);
...
export function MetroPlanner() {
  ...
  return (<>
    <Picker label="From" value={from} onPick={(v) => { setFrom(v); setSel(0); }}
      placeholder="Search station…" nearBtn onNear={() => useNear(setFrom)} />
    ...
    <Picker label="To" value={to} onPick={(v) => { setTo(v); setSel(0); }}
      placeholder="Search station…" nearBtn onNear={() => useNear(setTo)} />

    <div className="btnrow">
      <button className={`cat ${smart ? 'on' : ''}`} onClick={() => setSmart(!smart)}>💳 Smart card</button>
      <button className={`cat ${holiday ? 'on' : ''}`} onClick={() => setHoliday(!holiday)}>🎉 Sunday/holiday</button>
    </div>
    ...
    {routes && (<>
        ...
        <div className="cats" style={{ marginTop: 12 }}>
          {routes.map((x, i) => (
            <button key={i} className={`cat ${sel === i ? 'on' : ''}`} onClick={() => setSel(i)}>
              {i === 0 ? '⚡ Fastest' : x.changes === 0 ? '🚉 Direct' : `${x.changes} change${x.changes > 1 ? 's' : ''}`}
              {' · '}{x.minutes}m
            </button>))}
        </div>)}

      <Card>
        <div className="chead">{r.from} → {r.to}</div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
          <div className="big gradtext">₹{r.fare}</div>
          {smart && <span className="dim sm">smart-card price</span>}
        </div>
        <div className="g3" style={{ marginTop: 12 }}>
          <div className="stat"><div className="v">{r.minutes}</div><div className="l">Minutes</div></div>
          <div className="stat"><div className="v">{r.stations}</div><div className="l">Stations</div></div>
          <div className="stat"><div className="v">{r.changes}</div><div className="l">Changes</div></div>
        </div>
        <div className="g2" style={{ marginTop: 8 }}>
          <div className="stat"><div className="v">{r.km}</div><div className="l">km</div></div>
          <div className="stat"><div className="v">₹{r.fareSmart}</div><div className="l">With smart card</div></div>
        </div>
      </Card>
          ...
          <div className="col" key={i}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
              <span style={{ width: 12, height: 12, borderRadius: 3, flex: '0 0 auto',
                background: leg.colour || 'var(--green)' }} />
              <b style={{ flex: 1 }}>{leg.line}</b>
              <span className="tag">{leg.count} stops</span>
            </div>
      ...
      {expand && (
        <div className="list" style={{ marginTop: 8 }}>
          {r.path.map((n, i) => {
            ...
            return (
              <div className="row" key={i} style={{ padding: '9px 14px' }}>
                <span style={{ width: 22, textAlign: 'center', flex: '0 0 auto',
                  color: isEnd ? 'var(--green)' : isChange ? 'var(--warn)' : 'var(--fg3)',
                  fontSize: isEnd ? 13 : 11 }}>
                  {isEnd ? '◉' : isChange ? '🔄' : '•'}</span>
                <div className="main">
                  <b style={{ fontSize: 13, color: isChange ? 'var(--warn)' : '' }}>{n}</b>
      ...
      <div className="src">
        <span className="dot" />
        <span>Route from OpenStreetMap DMRC data ({STATIONS.length} stations, {LINES.length} line
          branches, built {BUILT}). Fares are the official DMRC slabs effective 25 Aug 2025.</span>
      </div>
    </>)}
  </>);
}
    ...
    <Card>
      ...
      {near.map((s) => (
        <div className="kv" key={s.n}>
          <span>{s.n}</span>
          <b style={{ color: 'var(--green)' }}>{s.km < 1 ? `${Math.round(s.km * 1000)} m` : `${s.km.toFixed(1)} km`}</b>
        </div>))}
    </Card>

    <div className="chead" style={{ marginTop: 14 }}>Lines ({LINES.length})</div>
    <div className="list">
      {LINES.map((L, i) => (
        <div className="row" key={i}>
          ...
          <span className="tag">{L.s.length}</span>
        </div>))}
    </div>

    <div className="fld" style={{ marginTop: 14 }}>
      <label>All {STATIONS.length} stations</label>
    ...
    <div className="list">
      {list.slice(0, 80).map((s) => (
        <a className="row" key={s.n} target="_blank" rel="noreferrer"
          ...
          <div className="main"><b style={{ fontSize: 13 }}>{s.n}</b>
            <span className="dim sm">{s.l.join(' · ')}</span></div>
          {s.l.length > 1 && <span className="tag w">🔄 {s.l.length}</span>}
        </a>))}
    </div>
  </>);
}
