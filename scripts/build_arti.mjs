...
const KIND = { aartis: 'aarti', chalisa: 'chalisa', mantras: 'mantra', stotrams: 'stotra' };
const MIN_CHARS = { aarti: 160, chalisa: 200, mantra: 30, stotra: 140 };
...
console.log(`arti.json: ${out.length} items [aarti ${n('aarti')} | chalisa ${n('chalisa')} | mantra ${n('mantra')} | stotra ${n('stotra')}] · ${Math.round(bytes / 1024)} KB raw · dropped ${JSON.stringify(drop)}`);
...
if (out.length < 200) { console.error('FAIL: corpus too small — check scrape run'); process.exit(1); }
